          ...
          <table class=" main-table table table-bordered text-center my-2">
            <thead class="">
              ...
              <tr>
                ...
                <td></td>
                <td>
                  <a href="?go=edit&id=<?=$row->user_ID?>" class="btn btn-warning confirm my-2">Edit</a>
                  <a href="?go=delete&id=<?=$row->user_ID?>" class="btn btn-danger confirm">Delete</a>
                </td>
              </tr>
           <?php }?>
   ...
   {    // add members                   ?>  
         ...
         <form action="?go=insert" class="form-horisontal " method='POST'>
          ...
          <div class="form-group col-sm-7 col-md-8">
           ...
           <i class="fa fa-eye fa-2px show-pass"></i>
   ...
   elseif($go=='insert')
    {
      if($_SERVER['REQUEST_METHOD']=='POST')
     {
      ...
      $stmt=$con->prepare('INSERT into users  (username,password,email,fullname ) values(:username,:password,:email,:fullname)');
      $stmt->bindParam(":username", $username);
      $stmt->bindParam(":password", $hash_pass);
      $stmt->bindParam(":email",$email);
      $stmt->bindParam(":fullname", $fullname);
      $stmt->execute();
      echo "<div class='alert alert-success'> The member was addes succesfully .</div>";
    
      }
      echo "</div> ";


   }else { redirectHome('hh',3);}
    }
  ...
  }elseif($go=='delete')
   {
    ...
    {  
      ...
      $stmt->execute();  ?>
         ...
         echo "<div class='alert alert-success'>The memeber has been deleted successfully</div>";
    
   <?php }else   echo "<div class='alert alert-warning'>This member does not exist</div>";
       echo "<div/> ";
   }
